...
def ejecutar_ensayo(ser):
    ...
    ws_param.append(["Variable", "y0", "A", "k", "R²", "Media", "Mediana", "Std", "Var", "Min", "Max"])
    # Gas
    ws_param.append([
        "Gas",
        popt_gas[0], popt_gas[1], popt_gas[2], r2_gas,
        stats_gas["media"], stats_gas["mediana"], stats_gas["std"], stats_gas["var"], stats_gas["min"], stats_gas["max"]
    ])
    # Humedad
    ws_param.append([
        "Humedad",
        popt_hum[0], popt_hum[1], popt_hum[2], r2_hum,
        stats_hum["media"], stats_hum["mediana"], stats_hum["std"], stats_hum["var"], stats_hum["min"], stats_hum["max"]
    ])
    # Temperatura (sin ajuste)
    ws_param.append([
        "Temperatura",
        "", "", "", "",
        stats_temp["media"], stats_temp["mediana"], stats_temp["std"], stats_temp["var"], stats_temp["min"], stats_temp["max"]
    ])
    # Presión (sin ajuste)
    ws_param.append([
        "Presion",
        "", "", "", "",
        stats_pres["media"], stats_pres["mediana"], stats_pres["std"], stats_pres["var"], stats_pres["min"], stats_pres["max"]
    ])
    ...
    if len(xdata) > 0 and not np.isnan(popt_gas).all():
    ...
    if len(xdata) > 0 and not np.isnan(popt_hum).all():
